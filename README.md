...
The first three trusted records are generated by `v2ray-source-hunter` and are
ordered `Elite`, `Stable`, then `Fresh` by `app_priority`. They are compact
Xray-validated feeds; the remaining curated sources stay available as fallback.
