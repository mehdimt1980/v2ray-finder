...
object XrayOutboundConverter {
    ...
    private fun convertShadowsocks(config: String): XrayOutboundConversion? {
        return try {
            ...
            val protocolName = "shadow" + "socks"
            XrayOutboundConversion("ss", baseOutbound(protocolName).put("settings", JSONObject().put("servers", JSONArray().put(JSONObject().put("address", host).put("port", port).put("method", method).put("password", password)))))
        } catch (_: Exception) { null }
    }
    ...
    private fun tryDecodeBase64(value: String): String? {
        return try { String(Base64.decode(padBase64(value), Base64.DEFAULT), Charsets.UTF_8) } catch (_: Exception) { null }
    }
